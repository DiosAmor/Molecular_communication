function [mol_position_totalstep, mol_arrive_count] = sim_3d_PoS_Uniformflow( ...
...
V                       = velocity_inMicroMeterPerSecond;
...
for t=2:sim_step_cnt
    ...
    % 'uniformflow' shows the velocity profile which is the same for all position.
    mol_displace = normrnd (0, sigma, ntx , 3);             % diffusion
    mol_displace_v = [V*delta_t*ones(ntx,1) zeros(ntx,2)];  % drift
